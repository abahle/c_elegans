...
%[map,M] = fast_extract(filein,varargin)
    ...
    for ii = 1:300%length(names)
        ...
        IM = process(IM,thresh,15,1); % process 
        IM = bwlabel(IM);
        ...
        center = bwmorph(center,'spur',10);
        
        ends = bwmorph(center,'endpoints'); % get endpoints
        
        if ii == 1
            map{ii}.head = endpointvalues(ends,center,Ori,200); %
        else
            [y,x] = find(ends == 1);
            X = [y,x];
        ...
        [map{ii}.boundary,map{ii}.center,map{ii}.hrot] = align(theta,boundary,center,map{ii}.head); % align
...
%             plot(map{ii}.hrot(2,:),map{ii}.hrot(1,:),'b.','MarkerSize',10), axis equal % plot ends points
                ...
                subplot(3,2,1:2:3)
                    imagesc(Ori), axis equal, axis off
                subplot(3,2,2:2:4)

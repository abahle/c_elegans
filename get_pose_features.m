...
function [angles, head_angles, bound, cent, hd,map,v] = get_pose_features(nfiles,dnsample)
...
%% Start analysis loop    
    for ii = 1:nfiles       
        ...
        IM = bwlabel(IM);
        IM = padarray(IM,[1,1]); % zero pad with one pixel to eliminate edge cases
        Ori = padarray(Ori,[1,1],'replicate');
        ...
        if ii == 1 || ii > 10 && ~isempty(find(v(1,ii-10:ii) >= 49000,1))
            headout = analysis.endpointvalues(ends,center,(Ori.*IM),20);
        else
            [y,x] = find(ends == 1);
            X = [y,x];
            headout = X(knnsearch(X,hd{ii-1}),:);
        end
